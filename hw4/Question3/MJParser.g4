parser grammar MJParser;

options { tokenVocab=MJLexer; }
...
mainClass: 'class' identifier '{' 'public' 'static' 'void' 'main' '(' 'String' '[' ']' identifier ')' '{' state=statement '}' '}'
    {print($state.value_attr)};

classDeclaration: 'class' identifier ( 'extends' identifier )? '{' ( varDeclaration )* ( methodDeclaration )* '}';

varDeclaration: t=typ i=identifier ';';

methodDeclaration: 'public' typ identifier '(' ( typ identifier ( ',' typ identifier )* )? ')' '{' ( varDeclaration )* ( state=statement {print($state.value_attr)} )* 'return' expression ';' '}';
